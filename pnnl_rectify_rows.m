function Y = pnnl_rectify_rows(X)
    % pnnl_rectify_rows Rectify the rows of a matrix.
    %
    %   Y = pnnl_rectify_rows(X) rectifies the rows of X so that the
    %   largest element of each row is non-negative.
    %
    %   Example:
    %
    %     X = [1, 2, -3;-6, 5, 4;7, -9, 8]
    %     Y = pnnl_rectify_rows(X)

    % Copyright 2022-2023 Battelle Memorial Institute
    abs_x = abs(X);
    Y = zeros(size(X));
    for k = 1:size(X,1)
        max_abs_index = find(abs_x(k,:) == max(abs_x(k,:)),1);
        sign_of_max = sign(X(k,max_abs_index));
        Y(k,:) = sign_of_max * X(k,:);
    end
end
% Reference
