function pure_spectra = pnnl_napalm_pure_spectra
    %pnnl_napalm_pure_spectra Napalm pure spectra
    %
    %   pnnl_napalm_pure_spectra returns the pure spectra from the training
    %   set.  The training set cannot contain pure polystyrene because it
    %   would be a solid, so it was mixed with benzene.  The pure
    %   polystyrene spectra is computed by subtracting out the proportional
    %   benzene spectrum from the mixture of benzene and polystyrene.
